    ...
    <div className={`social-card p-4 mb-4 ${calmMode ? 'bg-calm-card border-calm-border' : ''}`}>
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center">
          ...
          <h2 className={`text-lg font-semibold ${calmMode ? 'text-calm-text' : 'text-foreground'}`}>
            {liveSessions.length > 0 ? 'Live Now' : 'Live Sessions'}
          </h2>
          <span className={`ml-2 text-sm ${calmMode ? 'text-calm-muted' : 'text-muted-foreground'}`}>
            {liveSessions.length > 0 ? `(${liveSessions.length})` : '(Coming Soon)'}
          </span>
        </div>
      ...
      {isExpanded && (
        <>
          {hasAnySessions ? (
            ...
            <div className={`text-center py-8 ${calmMode ? 'text-calm-muted' : 'text-muted-foreground'}`}>
              <Video className={`h-12 w-12 mx-auto mb-4 opacity-50 ${calmMode ? 'text-calm-muted' : ''}`} />
              <p className="text-lg mb-2">No live sessions right now</p>
              <p className="text-sm">Check back soon for upcoming live sessions!</p>
            </div>
          )}
        </>
      )}
