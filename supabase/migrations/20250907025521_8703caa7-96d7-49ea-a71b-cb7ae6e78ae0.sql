...
-- Drop all existing functions that will have different return types
DROP FUNCTION IF EXISTS public.get_public_profile_discovery(uuid);
DROP FUNCTION IF EXISTS public.get_friend_profile_data(uuid);
DROP FUNCTION IF EXISTS public.get_safe_profile_discovery(uuid);

-- Create the new function with minimal essential information only for public discovery
...
-- Create friend profile function with appropriate fields for friends
